...
class PodcastFeed(Rss201rev2Feed):
    ...
    def add_item_elements(self, handler, item):
        ...
        # Sigh. The original code using the private `_write` does not play well with `_short_empty_elements`.
        # Hackily override that option here.
        # See: https://github.com/python/cpython/blob/5e0747db2f5a063657a117e88b58a20624c848d1/Lib/xml/sax/saxutils.py#L209-L213
        handler._short_empty_elements = False

        for key in DEFAULT_ITEM_ELEMENTS:
            # empty attributes will be ignored.
            if item[key] is None:
                continue
            if key in ('content:encoded', 'description'):
                content = item[key]
